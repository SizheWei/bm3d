...
sigma = 25
threshold_hard = 2.7 * sigma  # Threshold for Hard Thresholding

Size_Step1_blk = 4  # block_Size即块的大小
...
First_Match_threshold = 125 * Size_Step1_blk ** 2  # 用于计算block之间相似度的阈值
...
Step2_Blk_Step = 1
Step2_Search_Step = 1
Second_Match_threshold = 220. / 16 * Size_Step2_blk ** 2  # 用于计算block之间相似度的阈值
...
def Step1_fast_match(_noisyImg, _BlockPoint):
    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    dct_img = cv2.dct(img.astype(numpy.float64))  # 对目标block做二维余弦变换
    ...
    *要将_similar_blocks第三维依次取出,然在频域用阈值滤波之后,再作反变换
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            ...
            # 硬阈值变换，去掉较小的频率成分
            tem_Vct_Trans[numpy.abs(tem_Vct_Trans[:]) < threshold_hard] = 0.
...
def BM3D_step1(_noisyImg):
    ...
    block_Size = Size_Step1_blk  # 块大小
    blk_step = Step1_Blk_Step  # N块步长滑动
    # 根据步长确定搜索的次数 ？？？分子中是不是少减了1？？？
    ...
    for i in range(int(Width_num + 2)):
        for j in range(int(Height_num + 2)):
            ...
            # 添加另外两个通道的相似块集合
            Similar_Blks, Positions, Count = Step1_fast_match(yImg, m_blockPoint)  # 相似块集合、相似块位置、相似块数量
            ...
            Similar_Blks_U, statis_nonzero_U = Step1_3DFiltering(Similar_Blks_U)  # 协同过滤后的相似块集合、非零项数量
            Similar_Blks_V, statis_nonzero_V = Step1_3DFiltering(Similar_Blks_V)  # 协同过滤后的相似块集合、非零项数量
...
#     noise = numpy.matlib.randn(img.shape[:,:,0]) * sigma
#     res = img + noise
#     return res
...
if __name__ == '__main__':
    cv2.setUseOptimized(True)  # OpenCV 开启优化
    img_name = "images/eGrass.jpg"  # 图像的路径
    ori = cv2.imread(img_name,1)  # 读入图像，cv2.IMREAD_GRAYSCALE:以灰度模式读入图像
